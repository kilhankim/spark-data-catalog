...
public class StackOverFlowSurvey {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SparkSession session = SparkSession.builder().appName("StackOverFlowSurvey").config("hive.metastore.client.factory.class","com.amazonaws.glue.catalog.metastore.AWSGlueDataCatalogHiveClientFactory").master("local[1]").getOrCreate();
...
/*
        ...
        // master("local[1]") =>local[K] : Run Spark locally with K worker threads (ideally, set this to the number of cores on your machine).

        DataFrameReader dataFrameReader = session.read();

        //Dataset<Row> responses = dataFrameReader.option("header","true").csv("in/2016-stack-overflow-survey-responses.csv");
        Dataset<Row> responses = dataFrameReader.option("header","true").csv("s3n://aws-emr-stackoverflow-analytics-kkh/2016stackoverflowsurveyresponses.csv");
        //s3://aws-emr-stackoverflow-analytics-kkh/survey_results_public.csv
        //s3://aws-emr-stackoverflow-analytics-kkh/2016stackoverflowsurveyresponses.csv
        Catalog catalog = responses.sparkSession().catalog();
        System.out.println(catalog);
        System.out.println(catalog.currentDatabase());	// default
        System.out.println(catalog.listDatabases());
        Dataset<Database> result = catalog.listDatabases();
        
        Dataset<Table> tables = catalog.listTables();

   
        tables.collectAsList().forEach(table -> System.out.println("result : " + table.name()));
        System.out.println(tables);	// default
